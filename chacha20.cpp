   ...
   public:
    std::vector<uint8_t> encode(const std::vector<uint8_t>& plaintext)
    {
        ...
        if (first_)
        {
            ...
            crypto_secretstream_xchacha20poly1305_init_push(&st, header, key_);
        ...
        if (crypto_secretstream_xchacha20poly1305_push(&st, static_cast<uint8_t*>(ciphertext.data()), &outlen, plaintext.data(), plaintext.size(), nullptr, 0, tag) != 0)
        {
            return {};
        }
...
class chacha20_decrypt::chacha20_decrypt_impl
{
   ...
   public:
    std::vector<uint8_t> decode(const std::vector<uint8_t>& ciphertext)
    {
        ...
        if (first_)
        {
            return decode_header(ciphertext);
        }
        return decode_payload(ciphertext);
    }

   private:
    std::vector<uint8_t> decode_payload(const std::vector<uint8_t>& ciphertext)
    {
    ...
    std::vector<uint8_t> decode_header(const std::vector<uint8_t>& ciphertext)
    {
        ...
        if (crypto_secretstream_xchacha20poly1305_init_pull(&st, header, key_) != 0)
        {
            LOG_ERROR("failed to init pull");
        }
        first_ = false;
        if (header_.size() == crypto_secretstream_xchacha20poly1305_HEADERBYTES)
        {
...
std::vector<uint8_t> chacha20_encrypt::encode(const std::vector<uint8_t>& plaintext) { return impl_->encode(plaintext); }
...
std::vector<uint8_t> chacha20_decrypt::decode(const std::vector<uint8_t>& ciphertext) { return impl_->decode(ciphertext); }
